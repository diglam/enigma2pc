...
class AVSwitch:
	...
	def getOutputAspect(self):
		...
		if valstr in ("4_3_letterbox", "4_3_panscan"): # 4:3
			...
			try:
				if "1" in open(eEnv.resolve("${sysconfdir}/stb/vmpeg/0/aspect"), "r").read():# 4:3
					return (4,3)
			except IOError:
				pass
...
def InitAVSwitch():
	...
	"scale": _("Just scale"), "zoom": _("Zoom")}
	try:
		if "auto" in open(eEnv.resolve("${sysconfdir}/stb/video/policy_choices")).read():
			# TRANSLATORS: (aspect ratio policy: always try to display as fullscreen, when there is no content (black bars) on left/right, even if this breaks the aspect.
	...
	try:
		SystemInfo["CanDownmixAC3"] = "downmix" in open(eEnv.resolve("${sysconfdir}/stb/audio/ac3_choices"), "r").read()
	except:
	...
	try:
		SystemInfo["CanDownmixDTS"] = "downmix" in open(eEnv.resolve("${sysconfdir}/stb/audio/dts_choices"), "r").read()
	except:
	...
	if SystemInfo["CanDownmixDTS"]:
 	...
 	try:
		SystemInfo["CanDownmixAAC"] = "downmix" in open(eEnv.resolve("${sysconfdir}/stb/audio/aac_choices"), "r").read()
	except:
		SystemInfo["CanDownmixAAC"] = False

	if SystemInfo["CanDownmixAAC"]:
		def setAACDownmix(configElement):
			open(eEnv.resolve("${sysconfdir}/stb/audio/aac"), "w").write(configElement.value and "downmix" or "passthrough")
		config.av.downmix_aac = ConfigYesNo(default = True)
		config.av.downmix_aac.addNotifier(setAACDownmix)

	try:
		SystemInfo["CanChangeOsdAlpha"] = open(eEnv.resolve("${sysconfdir}/stb/video/alpha"), "r") and True or False
	except:
